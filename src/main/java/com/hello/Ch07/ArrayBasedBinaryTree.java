...
/**
 * An array based implementation of a binary tree. Javadoc comments for methods
 * specified in the BinaryTree interface have been omitted.
 *
...
public class ArrayBasedBinaryTree<T> implements BinaryTree<T> {
	...
	/**
	 * Compare the content and structure of this tree to a second binary tree.
	 *
	 * @param tree
	 *            the tree to compare this tree with
	 *
	 * @return true if the content and structure of a given tree is identical to
	 *         this tree and false otherwise.
	 */
	 ...
	 *
	 * @return the location of the node that contains the target, or OFF_TREE if
	...
	private int search(int root, T target) {
		...
		if (location == OFF_TREE && children[root][RIGHT] != OFF_TREE) {
	...
	/**
	 * Return a string representation of this tree. The string returned by this
	 * method will show the structure of the tree if the string is rotated 90
	...
	/**
	 * A recursive method that does a RVL traversal of the tree to create a
	 * string that shows the contents and structure of the tree.
	 *
	...
	/**
	 * Return the position of the parent given a root of the tree and the child.
	 *
	 * @param root
	 *            the root of the tree to sreach.
	 * @param child
	 *            the child whose parent we are looking for.
	 *
	 * @return the location of the parent in the tree or OFF_TREE if the parent
	 *         cannot be found.
	 */
	protected int findParent(int root, int child) {
		...
		if (children[root][LEFT] == child || children[root][RIGHT] == child) {
			parent = root;
		} else {
			...
			if (parent == OFF_TREE && children[root][RIGHT] != OFF_TREE) {
	...
	/**
	 * Locate the next open index in the arrays holding the tree nodes. If there
	 * is not enough room in the arrays for the new node, the arrays will be
	 * expanded.
	 *
	...
	/**
	 * Double the capacity of the arrays that hold the data and the links.
	 */
	private void expand() {
	...
	/**
	 * Add the array locations occupied by the specified tree to the free list.
	 *
	 * @param root
