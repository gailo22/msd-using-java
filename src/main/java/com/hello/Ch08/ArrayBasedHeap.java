...
public class ArrayBasedHeap<T extends Comparable> implements Heap<T> {
	 ...
	 *
	 * Preconditions: data is not null. data is of the same data type as the
	 * other nodes in the heap.
	 *
	 * Postconditions: data has been added to the heap. The heap property has
	 * been preserved.
	 * 
	 * @param data
	...
	/**
	 * Ensure that the ordering property of the heap holds after the insertion
	 * of an new element into the heap.
	 */
	private void interchangeUp() {
		...
		if (size > 1) {
			...
			while (parent >= 0 && heapForm[cur].compareTo(heapForm[parent]) < 0) {
	 ...
	 * 
	 * Preconditions: The heap is not empty.
	 *
	 * Postconditions: The element has been removed.
	 *
	...
	/**
	 * Make sure the ordering property holds after removing the smallest element
	 * from the heap.
	 */
	 ...
	 *         Preconditions: None
	 * 
	 *         Postconditions: The heap is unchanged.
	 *
