...
public class Traversals implements Callback<Vertex> {
	 ...
	 * @param start
	 *            the vertex where the traversal will start
	 * @param cb
	 *            the object that processes vertices.
	 */
	 ...
	 *
	 * @param g
	 *            the graph from which to generate the MST
	 *
	...
	public Graph kruskalMST(Graph g) {
		...
		while (vertexSets.size() > 1) {
			...
			if (sourcePos != destPos) {
				...
				mst.addEdge(cur.getSource().getName(), cur.getDest().getName(),
						cur.getCost());
	 ...
	 *            the Graph to traverse.
	 * @param name
	 *            the name of the vertex where the traversal starts.
	 * 
