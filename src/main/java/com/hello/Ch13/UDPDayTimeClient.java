...
/**
 * A simple client that will query the daytime service on a remote to determine
 * the date and time at that location.
 */
		...
		if (args.length != 2) {
			...
			try {
				...
				DatagramPacket packet = new DatagramPacket(
						new byte[MAX_PACKET_SIZE], MAX_PACKET_SIZE, server,
						port);
				...
				serverTime = new String(packet.getData(), 0,
						packet.getLength() - 1, "US-ASCII");
