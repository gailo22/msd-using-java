...
/**
 * A data input screen that allows students to selected courses. Students select
 * from 1 to 3 courses and then press the finish button. The names of the
 * courses wil be displayed in the text area.
 */
public class GUIExample1 extends JFrame implements ItemListener, ActionListener {
	...
	private static final String courses[] = { "CS 101", "CS 102", "CS 210",
			"CS 215", "CS 217", "CS 302" };
	 ...
	 *
	 * @param title
	 *            the title to be placed in the frame.
	 */
	public GUIExample1(String title) {
		...
		JLabel l1 = new JLabel("Please select the courses you wish "
				+ " to take.  Click on the Finished button " + "when done.");
		...
		display.setBorder(BorderFactory.createTitledBorder(
				BorderFactory.createEtchedBorder(), "Courses Selected"));
	...
	/**
	 * This method will be called whenever the user clicks on a check box. If
	 * the user is selecting a course it will be added to the set of selected
	 * courses. If they are deselecting a course it will be removed from the
	 * set.
	 *
	 * @param event
	 *            the item event that caused this method to be invoked.
	 */
	 ...
	 * button. If between 1 and 3 courses have been selected, the names of the
	 * course are displayed in the GUI. If the incorrect number of courses is
	 * selected an error message is displayed.
	 *
	 * @param even
	 *            the action event that caused this method to be invoked.
	 */
