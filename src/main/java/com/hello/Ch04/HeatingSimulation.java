...
/**
 * The program that runs the heating simulation. The heating system modeled by
 * this program consists of a single room heated by one furnace which is
...
public class HeatingSimulation {
	...
	private static double simParams[] = { 72.0, // Inside temperature
	...
	public static void main(String args[]) {
		...
		theRoom = new Room(theWorld, theFurnace, simParams[ROOM_SIZE],
		...
		// displayed
		System.out.println("Starting simulation:  " + " display frequency="
				+ simParams[DISPLAY_FREQ] + " runtime(sec.)= "
				+ simParams[SIM_LENGTH] + "\n");

		System.out.println("Time\tInside\tOutside\tDesired\tFurnace");
		System.out.println("----\t------\t-------\t-------\t-------");

		for (int simTime = 0; simTime <= (int) simParams[SIM_LENGTH]; simTime = simTime
				+ (int) simParams[DISPLAY_FREQ] * SECS_BETWEEN_EVENTS) {
	...
	/**
	 * Scan the command line arguments and set any simulation parameters as
	 * specified by the user. Invalid parameters settings will be ignored. Note
	 * that if an invalid numeric value is specified on the command line a
	 * runtime exception will be thrown and the program will terminate.
	 *
	 * @param args
	 *            the parameter settings to parse.
	 */
